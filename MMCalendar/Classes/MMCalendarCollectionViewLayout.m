...
#define kMMCalendarSeparatorInterRows @"MMCalendarSeparatorInterRows"
#define kMMCalendarSeparatorInterColumns @"MMCalendarSeparatorInterColumns"

@interface MMCalendarCollectionViewLayout ()
...
@implementation MMCalendarCollectionViewLayout
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self registerClass:[MMCalendarSeparator class] forDecorationViewOfKind:kMMCalendarSeparatorInterRows];
    }
    return self;
}
...
- (void)prepareLayout
{
    ...
    self.estimatedItemSize = ({
        ...
        CGFloat height = ({
            CGFloat height = MMCalendarStandardRowHeight;
            if (!self.calendar.floatingMode) {
    ...
    self.widths = ({
        ...
        MMCalendarSliceCake(contentWidth, columnCount, widths);
        widths;
    });
    ...
    free(self.heights);
    self.heights = ({
        NSInteger rowCount = self.calendar.transitionCoordinator.representingScope == MMCalendarScopeWeek ? 1 : 6;
        ...
        if (!self.calendar.floatingMode) {
            ...
            MMCalendarSliceCake(contentHeight, rowCount, heights);
        } else {
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if (!self.calendar.floatingMode) {
        ...
        switch (self.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                ...
                NSInteger startColumn = ({
                    ...
                    CGFloat widthDelta = MMCalendarMod(CGRectGetMinX(rect), self.collectionView.fs_width)-self.sectionInsets.left;
                    widthDelta = MIN(MAX(0, widthDelta), self.collectionView.fs_width-self.sectionInsets.left);
                    NSInteger countDelta = MMCalendarFloor(widthDelta/self.estimatedItemSize.width);
                ...
                NSInteger endColumn = ({
                    ...
                    CGFloat remainder = MMCalendarMod(section, 1);
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endColumn = MMCalendarFloor(section)*7 - 1;
                    } else {
                        CGFloat widthDelta = MMCalendarMod(CGRectGetMaxX(rect), self.collectionView.fs_width)-self.sectionInsets.left;
                        widthDelta = MIN(MAX(0, widthDelta), self.collectionView.fs_width - self.sectionInsets.left);
                        NSInteger countDelta = MMCalendarCeil(widthDelta/self.estimatedItemSize.width);
                        endColumn = MMCalendarFloor(section)*7 + countDelta - 1;
                    }
                    endColumn;
                });
                
                NSInteger numberOfRows = self.calendar.transitionCoordinator.representingScope == MMCalendarScopeMonth ? 6 : 1;
                ...
                for (NSInteger column = startColumn; column <= endColumn; column++) {
                    for (NSInteger row = 0; row < numberOfRows; row++) {
                        ...
                        UICollectionViewLayoutAttributes *rowSeparatorAttributes = [self layoutAttributesForDecorationViewOfKind:kMMCalendarSeparatorInterRows atIndexPath:indexPath];
            ...
            case UICollectionViewScrollDirectionVertical: {
                ...
                NSInteger startRow = ({
                    ...
                    CGFloat heightDelta = MMCalendarMod(CGRectGetMinY(rect), self.collectionView.fs_height)-self.sectionInsets.top;
                    heightDelta = MIN(MAX(0, heightDelta), self.collectionView.fs_height-self.sectionInsets.top);
                    NSInteger countDelta = MMCalendarFloor(heightDelta/self.estimatedItemSize.height);
                ...
                NSInteger endRow = ({
                    ...
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endRow = MMCalendarFloor(section)*6 - 1;
                    } else {
                        CGFloat heightDelta = MMCalendarMod(CGRectGetMaxY(rect), self.collectionView.fs_height)-self.sectionInsets.top;
                        heightDelta = MIN(MAX(0, heightDelta), self.collectionView.fs_height-self.sectionInsets.top);
                        NSInteger countDelta = MMCalendarCeil(heightDelta/self.estimatedItemSize.height);
                        endRow = MMCalendarFloor(section)*6 + countDelta-1;
                    }
                    endRow;
                });
        ...
        NSInteger startRowIndex = ({
            ...
            NSInteger startRowCount = MMCalendarCeil(heightDelta1/self.estimatedItemSize.height);
        ...
        NSInteger endRowIndex = ({
            ...
            NSInteger endRowCount = MMCalendarCeil(heightDelta2/self.estimatedItemSize.height);
...
{
    MMCalendarCoordinate coordinate = [self.calendar.calculator coordinateForIndexPath:indexPath];
...
{
    if ([elementKind isEqualToString:kMMCalendarSeparatorInterRows] && (self.separators & MMCalendarSeparatorInterRows)) {
        ...
        if (!attributes) {
            ...
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:kMMCalendarSeparatorInterRows withIndexPath:indexPath];
            CGFloat x, y;
            if (!self.calendar.floatingMode) {
                switch (self.scrollDirection) {
                    case UICollectionViewScrollDirectionHorizontal: {
            ...
            CGFloat width = self.collectionView.fs_width;
            CGFloat height = MMCalendarStandardSeparatorThickness;
...
#undef kMMCalendarSeparatorInterColumns
#undef kMMCalendarSeparatorInterRows
