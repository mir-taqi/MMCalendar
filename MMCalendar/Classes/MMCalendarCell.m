...
{   
    ...
    MMCalendarEventIndicator *eventIndicator;
    ...
    eventIndicator = [[MMCalendarEventIndicator alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews
{
    ...
    if (_subtitle) {
        ...
        _titleLabel.frame = CGRectMake(
                                       ...
                                       self.contentView.fs_width,
                                       self.contentView.fs_height//floor(self.contentView.fs_height*5.0/6.0)
                                       );
    }
    ...
    diameter = diameter > MMCalendarStandardCellDiameter ? (diameter - (diameter-MMCalendarStandardCellDiameter)*0.5) : diameter;
...
#define kAnimationDuration MMCalendarDefaultBounceAnimationDuration
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary
{
    if (self.isSelected) {
        if (self.dateIsToday) {
            return dictionary[@(MMCalendarCellStateSelected|MMCalendarCellStateToday)] ?: dictionary[@(MMCalendarCellStateSelected)];
        }
        return dictionary[@(MMCalendarCellStateSelected)];
    }
    if (self.dateIsToday && [[dictionary allKeys] containsObject:@(MMCalendarCellStateToday)]) {
        return dictionary[@(MMCalendarCellStateToday)];
    }
    if (self.placeholder && [[dictionary allKeys] containsObject:@(MMCalendarCellStatePlaceholder)]) {
        return dictionary[@(MMCalendarCellStatePlaceholder)];
    }
    if (self.weekend && [[dictionary allKeys] containsObject:@(MMCalendarCellStateWeekend)]) {
        return dictionary[@(MMCalendarCellStateWeekend)];
    }
    return dictionary[@(MMCalendarCellStateNormal)];
}
...
@interface MMCalendarEventIndicator ()
...
@implementation MMCalendarEventIndicator
...
{
    [super layoutSubviews];
    CGFloat diameter = MIN(MIN(self.fs_width, self.fs_height),MMCalendarMaximumEventDotDiameter);
...
@implementation MMCalendarBlankCell
